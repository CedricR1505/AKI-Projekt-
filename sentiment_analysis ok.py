...
def parse_feed_item(item_xml: str, source_name: str) -> dict:
    ...
    return {"title": title, "date": pub_date, "source": original_source}
...
def fetch_news_from_feeds(symbol: str, period: str = "1mo", news_limit: int = 100):
    ...
    print(f"[Sentiment] Insgesamt {len(news_items)} News aus: {sources_found}")
...
def create_sentiment_chart(symbol: str, hist, sentiment_daily) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(x=hist.index, y=hist["Close"], mode="lines", name=f"{symbol} Kurs",
                   line=dict(color=color_line, width=2), hovertemplate="%{y:.2f} USD<extra></extra>"),
        secondary_y=False
    )
    ...
    if len(sentiment_daily) > 0:
        ...
        fig.add_trace(
            go.Bar(x=pd.to_datetime(sentiment_daily.index), y=sentiment_daily.values, name="Sentiment Score",
                   marker_color=colors_bars, opacity=0.5, hovertemplate="Sentiment: %{y:.2f}<extra></extra>"),
            secondary_y=True
        )
    ...
    fig.update_layout(
        title=dict(text=f"{symbol} Kurs vs. Nachrichten-Stimmung ({sign}{pct_change:.2f}%)", font=dict(size=16)),
        plot_bgcolor="white", paper_bgcolor="white", hovermode="x unified",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5),
        margin=dict(l=50, r=50, t=80, b=50), height=400
    )
...
def create_correlation_chart(symbol: str, merged_df) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=merged_df["date"], y=merged_df["price"], mode="lines", name="Kurs",
                             line=dict(color=color_price, width=2), hovertemplate="%{y:.2f} USD<extra>Kurs</extra>"), row=1, col=1)
    ...
    for i in range(len(merged_df) - 1):
        ...
        if abs(sentiment_val) > 0.1:
            ...
            fig.add_vrect(x0=merged_df["date"].iloc[i], x1=merged_df["date"].iloc[i+1],
                          fillcolor=fill_color, layer="below", line_width=0, row=1, col=1)
    
    colors_bars = ["#22c55e" if s > 0 else "#ef4444" for s in merged_df["sentiment_ma"]]
    fig.add_trace(go.Bar(x=merged_df["date"], y=merged_df["sentiment_ma"], name="Sentiment (MA7)",
                         marker_color=colors_bars, opacity=0.7, hovertemplate="Sentiment: %{y:.3f}<extra></extra>"), row=2, col=1)
    
    fig.add_hline(y=0, line_dash="dash", line_color="gray", row=2, col=1)
    
    fig.update_layout(height=600, plot_bgcolor="white", paper_bgcolor="white", hovermode="x unified",
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5),
...
def analyze_sentiment(symbol: str, period: str = "1mo", news_limit: int = 100) -> dict:
    ...
    try:
        ...
        return {
            "success": True, "symbol": symbol, "news_items": news_items, "sources_found": sources_found,
            "sentiment_daily": sentiment_daily, "figure": fig,
            "stats": {
                "avg_sentiment": avg_sentiment, "news_count": len(news_items), "sentiment_days": len(sentiment_daily),
                "start_price": start_price, "end_price": end_price, "pct_change": pct_change,
                "is_positive": end_price >= start_price,
            }
        }
    except Exception as e:
...
def analyze_correlation(symbol: str, period: str = "3mo", news_limit: int = 500) -> dict:
    ...
    try:
        ...
        if len(news_items) < 5:
            return {"error": f"Zu wenige News für '{symbol}' gefunden ({len(news_items)} Artikel)."}
        ...
        return {
            "success": True, "symbol": symbol, "correlation": correlation, "figure": fig, "merged_df": merged_df,
            "stats": {
                "news_count": len(news_items), "days_back": days_back, "start_price": start_price,
                "end_price": end_price, "pct_change": pct_change, "is_positive": end_price >= start_price,
...
def analyze_forecast(symbol: str, history_period: str = "1y", forecast_days: int = 30) -> dict:
    ...
    try:
        ...
        if hist.empty or len(hist) < 30:
            return {"error": f"Nicht genügend Kursdaten für '{symbol}'."}
        ...
        df = pd.DataFrame({"Close": close_prices, "Date": dates_clean})
        df = df.dropna()
        ...
        time_factor = np.log1p(np.arange(1, forecast_days + 1)) / np.log1p(30)
        ci_margin = 1.96 * std_err * (1 + time_factor * daily_volatility * np.sqrt(252))
        
        if forecast_days > 365:
            long_term_uncertainty = (forecast_days / 365) * 0.1 * current_price
            ci_margin = ci_margin + long_term_uncertainty * time_factor / time_factor[-1]
        
        forecast_ci_lower = np.maximum(forecast_mean - ci_margin, current_price * 0.01)
        ...
        forecast_end_price = forecast_mean[-1] if not hasattr(forecast_mean, 'iloc') else forecast_mean.iloc[-1]
        ...
        ci_lower = forecast_ci[-1, 0] if not hasattr(forecast_ci, 'iloc') else forecast_ci.iloc[-1, 0]
        ci_upper = forecast_ci[-1, 1] if not hasattr(forecast_ci, 'iloc') else forecast_ci.iloc[-1, 1]
        
        return {
            "success": True, "symbol": symbol, "figure": fig,
            "stats": {
                "current_price": current_price, "forecast_price": forecast_end_price,
                "forecast_change": forecast_change, "forecast_trend": forecast_trend,
                "ci_lower": ci_lower, "ci_upper": ci_upper, "forecast_days": forecast_days,
                "history_days": len(df), "arima_order": best_order, "aic": best_aic,
                "annual_drift": annual_drift * 100, "annual_volatility": annual_volatility * 100,
            }
        }
    except Exception as e:
        import traceback
...
def create_forecast_chart(symbol: str, hist_df, forecast_dates, forecast_mean, forecast_ci, arima_order) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=hist_dates, y=hist_close, mode="lines", name="Historischer Kurs",
                             line=dict(color=color_hist, width=2), hovertemplate="%{y:.2f} USD<extra>Historisch</extra>"))
    
    forecast_values = list(forecast_mean) if hasattr(forecast_mean, '__iter__') else forecast_mean.tolist()
    color_forecast = "#3b82f6"
    
    fig.add_trace(go.Scatter(x=forecast_dates, y=forecast_values, mode="lines", name="Prognose",
                             line=dict(color=color_forecast, width=2, dash="dash"), hovertemplate="%{y:.2f} USD<extra>Prognose</extra>"))
    
    ci_upper = list(forecast_ci[:, 1]) if not hasattr(forecast_ci, 'iloc') else list(forecast_ci.iloc[:, 1])
    ci_lower = list(forecast_ci[:, 0]) if not hasattr(forecast_ci, 'iloc') else list(forecast_ci.iloc[:, 0])
    
    fig.add_trace(go.Scatter(x=list(forecast_dates) + list(reversed(forecast_dates)),
                             y=ci_upper + list(reversed(ci_lower)), fill="toself",
                             fillcolor="rgba(59, 130, 246, 0.2)", line=dict(color="rgba(255,255,255,0)"),
                             name="95% Konfidenzintervall", hoverinfo="skip"))
    ...
    fig.add_trace(go.Scatter(x=[last_hist_date, forecast_dates[0]], y=[last_hist_price, first_forecast_price],
                             mode="lines", line=dict(color=color_forecast, width=2, dash="dash"),
                             showlegend=False, hoverinfo="skip"))
    
    fig.add_shape(type="line", x0=last_hist_date, x1=last_hist_date, y0=0, y1=1, yref="paper",
                  line=dict(color="gray", dash="dot", width=1))
    fig.add_annotation(x=last_hist_date, y=1, yref="paper", text="Heute", showarrow=False, yanchor="bottom")
    
    forecast_change = ((forecast_values[-1] - last_hist_price) / last_hist_price) * 100
    sign = "+" if forecast_change >= 0 else ""
    
    fig.update_layout(
        title=dict(text=f"{symbol} Kursprognose - ARIMA{arima_order} ({sign}{forecast_change:.1f}% erwartet)", font=dict(size=16)),
        xaxis_title="Datum", yaxis_title="Kurs (USD)", plot_bgcolor="white", paper_bgcolor="white",
...
def analyze_monte_carlo(symbol: str, history_period: str = "1y", forecast_days: int = 30, num_simulations: int = 1000) -> dict:
    ...
    try:
        ...
        for t in range(1, forecast_days + 1):
            ...
            simulations[:, t] = simulations[:, t-1] * np.exp((mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * random_returns)
        
        final_prices = simulations[:, -1]
        percentiles = {
            "p5": np.percentile(final_prices, 5), "p10": np.percentile(final_prices, 10),
            "p25": np.percentile(final_prices, 25), "p50": np.percentile(final_prices, 50),
            "p75": np.percentile(final_prices, 75), "p90": np.percentile(final_prices, 90),
        ...
        fig = create_monte_carlo_chart(symbol, df, forecast_date_list, simulations, percentiles, mean_price, current_price)
        forecast_change = ((mean_price - current_price) / current_price) * 100
        
        return {
            "success": True, "symbol": symbol, "figure": fig, "simulations": simulations,
            "stats": {
                "current_price": current_price, "mean_price": mean_price, "median_price": percentiles["p50"],
                "std_price": std_price, "forecast_change": forecast_change, "forecast_days": forecast_days,
                "num_simulations": num_simulations, "history_days": len(df), "mu": mu * 252,
                "sigma": sigma * np.sqrt(252), "percentiles": percentiles, "prob_positive": prob_positive,
                "prob_up_10": prob_up_10, "prob_down_10": prob_down_10,
            }
        }
    except Exception as e:
        import traceback
...
def create_monte_carlo_chart(symbol: str, hist_df, forecast_dates, simulations, percentiles, mean_price, current_price) -> go.Figure:
    fig = go.Figure()
    ...
    for i in range(num_display):
        fig.add_trace(go.Scatter(x=forecast_dates, y=simulations[i, :], mode="lines",
                                 line=dict(color="rgba(100, 100, 100, 0.05)", width=0.5), showlegend=False, hoverinfo="skip"))
    
    p5_values = np.percentile(simulations, 5, axis=0)
    p95_values = np.percentile(simulations, 95, axis=0)
    fig.add_trace(go.Scatter(x=list(forecast_dates) + list(reversed(forecast_dates)),
                             y=list(p95_values) + list(reversed(p5_values)), fill="toself",
                             fillcolor="rgba(59, 130, 246, 0.15)", line=dict(color="rgba(255,255,255,0)"),
                             name="90% Konfidenzintervall", hoverinfo="skip"))
    
    p25_values = np.percentile(simulations, 25, axis=0)
    p75_values = np.percentile(simulations, 75, axis=0)
    fig.add_trace(go.Scatter(x=list(forecast_dates) + list(reversed(forecast_dates)),
                             y=list(p75_values) + list(reversed(p25_values)), fill="toself",
                             fillcolor="rgba(59, 130, 246, 0.25)", line=dict(color="rgba(255,255,255,0)"),
                             name="50% Konfidenzintervall", hoverinfo="skip"))
    
    median_values = np.percentile(simulations, 50, axis=0)
    color_forecast = "#3b82f6"
    fig.add_trace(go.Scatter(x=forecast_dates, y=median_values, mode="lines", name="Median (50%)",
                             line=dict(color=color_forecast, width=3), hovertemplate="%{y:.2f} USD<extra>Median</extra>"))
    
    mean_values = np.mean(simulations, axis=0)
    fig.add_trace(go.Scatter(x=forecast_dates, y=mean_values, mode="lines", name="Mittelwert",
                             line=dict(color="#f59e0b", width=2, dash="dash"), hovertemplate="%{y:.2f} USD<extra>Mittelwert</extra>"))
    ...
    fig.add_trace(go.Scatter(x=[last_hist_date, forecast_dates[0]], y=[last_hist_price, simulations[0, 0]],
                             mode="lines", line=dict(color=color_forecast, width=2, dash="dash"),
                             showlegend=False, hoverinfo="skip"))
    ...
    fig.update_layout(
        title=dict(text=f"{symbol} Monte-Carlo Simulation ({sign}{forecast_change:.1f}% erwartet)", font=dict(size=16)),
        xaxis_title="Datum", yaxis_title="Kurs (USD)", plot_bgcolor="white", paper_bgcolor="white",
        hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5),
        margin=dict(l=50, r=50, t=80, b=50), height=500
    )
    fig.update_xaxes(showgrid=True, gridcolor="#e5e7eb")
    fig.update_yaxes(showgrid=True, gridcolor="#e5e7eb")
